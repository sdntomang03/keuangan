<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between">
            ...
            <div class="mt-2 md:mt-0 flex gap-2 print:hidden">
                <button onclick="window.print()" class="bg-gray-800 text-white px-4 py-2 rounded-lg text-xs font-bold hover:bg-black transition flex items-center shadow-sm">
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            {{-- Navigasi Triwulan (Hidden saat print) --}}
            <div class="flex justify-center mb-6 print:hidden">
                <div class="inline-flex p-1.5 bg-gray-200/50 rounded-2xl border border-gray-200 shadow-inner backdrop-blur-sm">
                    @foreach(['tahun' => 'Tahunan', '1' => 'TW I', '2' => 'TW II', '3' => 'TW III', '4' => 'TW IV'] as $key => $label)
            ...
            {{-- Header Kop Surat --}}
            <div class="bg-white p-6 mb-6 rounded-xl shadow-sm border border-gray-200 text-center relative overflow-hidden">
                <div class="absolute top-0 left-0 w-full h-1 {{ $anggaran->singkatan == 'BOS' ? 'bg-indigo-600' : 'bg-emerald-600' }}"></div>
                <h3 class="text-xl font-black uppercase text-gray-800 tracking-wider">{{ $sekolah->nama_sekolah ?? 'NAMA SEKOLAH' }}</h3>
                <p class="text-sm text-gray-600 mt-1 uppercase">
                    LAPORAN REALISASI PENGGUNAAN DANA <span class="font-bold text-indigo-600">{{ $anggaran->singkatan }}</span>
                </p>
                ...
                <table class="w-full border-collapse text-[11px]">
                    <thead>
                        <tr class="bg-gray-800 text-white uppercase tracking-widest">
                            ...
                            {{-- Judul kolom menyesuaikan dengan TW --}}
                            <th class="px-4 py-4 text-right font-bold w-40">Anggaran {{ $tw == 'tahun' ? '' : 'TW '.$tw }}</th>
                            <th class="px-4 py-4 text-right font-bold w-40">Realisasi {{ $tw == 'tahun' ? '' : 'TW '.$tw }}</th>
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($dataRkas as $idbl => $perAkun)
                            ...
                            <tr class="bg-indigo-50/50 print:bg-gray-100">
                                ...
                                <td class="px-4 py-3 text-right font-black text-indigo-900 font-mono">{{ number_format($totalAnggaranGiat, 0, ',', '.') }}</td>
                                <td class="px-4 py-3 text-right font-black text-indigo-900 font-mono">{{ number_format($totalRealisasiGiat, 0, ',', '.') }}</td>
                                <td class="px-4 py-3 text-right font-black text-indigo-900 font-mono">{{ number_format($sisaGiat, 0, ',', '.') }}</td>
                                <td class="px-4 py-3 text-center font-black text-indigo-900">{{ number_format($persenGiat, 1) }}%</td>
                            </tr>
                            ...
                            @foreach($perAkun as $kodeakun => $items)
                                ...
                                <tr class="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td class="px-4 py-3 text-right text-gray-500 font-mono">{{ number_format($totalAnggaranRek, 0, ',', '.') }}</td>
                                    <td class="px-4 py-3 text-right font-bold text-gray-800 font-mono">{{ number_format($totalRealisasiRek, 0, ',', '.') }}</td>
                                    <td class="px-4 py-3 text-right font-bold font-mono {{ $sisaRek < 0 ? 'text-red-600' : 'text-emerald-600' }}">
                                        {{ number_format($sisaRek, 0, ',', '.') }}
                                    </td>
                                    <td class="px-4 py-3 text-center">
                                        <div class="text-[10px] font-black {{ $persenRek > 100 ? 'text-red-600' : 'text-gray-400' }}">
                            ...
                            @endforeach
                        @empty
                            <tr><td colspan="5" class="py-20 text-center text-gray-400 italic">Data tidak ditemukan untuk periode ini.</td></tr>
                        @endforelse
                    </tbody>

                    {{-- Footer Total --}}
                    @if($dataRkas->count() > 0)
                    <tfoot class="bg-gray-900 text-white font-bold uppercase tracking-widest border-t-4 border-indigo-500">
                        @php
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-5 text-right font-mono text-base">{{ number_format($grandAnggaran, 0, ',', '.') }}</td>
                            <td class="px-4 py-5 text-right font-mono text-base text-yellow-400">{{ number_format($grandRealisasi, 0, ',', '.') }}</td>
                            <td class="px-4 py-5 text-right font-mono text-base">{{ number_format($grandAnggaran - $grandRealisasi, 0, ',', '.') }}</td>
                            <td class="px-4 py-5 text-center text-xs">{{ number_format($totalPersen, 1) }}%</td>
                        </tr>
                    </tfoot>
                    @endif
                </table>
            ...
            <div class="hidden print:block mt-16">
                <div class="flex justify-around text-xs text-center font-bold">
                    <div class="w-1/3">
                        ...
                        <span class="border-b border-black px-4 ">{{ $sekolah->nama_kepala_sekolah ?? '................................' }}</span><br>
                        NIP. {{ $sekolah->nip_kepala_sekolah ?? '................................' }}
                    </div>
                    <div class="w-1/3">
                        {{ now()->translatedFormat('d F Y') }}<br>Bendahara,<br><br><br><br><br>
                        <span class="border-b border-black px-4">{{ $sekolah->nama_bendahara ?? '................................' }}</span><br>
    ...
    @media print {
        ...
        .rounded-xl, .rounded-\[2rem\] { border-radius: 0 !important; }
