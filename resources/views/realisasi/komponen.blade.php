        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                ...
                <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
                    ...
                    <p class="text-xl font-black text-indigo-600">Rp {{ number_format($grandRealisasi, 0, ',', '.') }}</p>
                </div>
                <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
            ...
            {{-- Filter & Search --}}
            <div class="bg-white p-4 mb-6 rounded-xl shadow-sm border border-gray-200 flex flex-col md:flex-row gap-4 print:hidden">
                <div class="relative flex-1">
                    <span class="absolute inset-y-0 left-0 pl-3 flex items-center text-gray-400">
                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" stroke-width="2"/></svg>
                    </span>
                    <input type="text" id="komponenSearch" class="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-indigo-500 focus:border-indigo-500" placeholder="Cari komponen belanja atau spesifikasi...">
                </div>
                <button onclick="window.print()" class="bg-gray-800 text-white px-6 py-2 rounded-lg text-sm font-bold flex items-center hover:bg-black transition shadow-md">
                ...
                <table class="w-full border-collapse text-[11px]">
                    <thead class="bg-gray-800 text-white uppercase tracking-wider">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-center font-bold">Vol. {{ $tw == 'tahun' ? 'Real/Pagu' : 'TW' }}</th>
                                ...
                                <td colspan="6" class="px-4 py-2">
                                    <div class="flex items-center">
                                        ...
                                        <span class="font-black text-indigo-900 text-[11px] uppercase">
                                            {{ $perAkun->first()->first()->kegiatan->namagiat ?? 'Kegiatan Tidak Terdefinisi' }}
                                        </span>
                                    </div>
                                </td>
                            </tr>
                                ...
                                @foreach($items as $item)
                                    ...
                                    <tr class="hover:bg-gray-50 transition border-b border-gray-100">
                                        ...
                                        <td class="px-4 py-3 text-center">
                                            <span class="font-mono font-bold {{ $volRealisasi > $volAnggaran ? 'text-red-600' : 'text-gray-700' }}">
                                        ...
                                        <td class="px-4 py-3 text-right font-mono font-bold {{ $sisaVal < 0 ? 'text-red-600' : 'text-emerald-600' }}">
                                            {{ number_format($sisaVal, 0, ',', '.') }}
                                        </td>
                                        <td class="px-4 py-3 text-center">
                                            <div class="flex flex-col items-center">
                                                <div class="w-12 bg-gray-200 rounded-full h-1 mb-1 overflow-hidden">
                                                    <div class="h-1 {{ $persen > 100 ? 'bg-red-500' : 'bg-indigo-500' }}" style="width: {{ min($persen, 100) }}%"></div>
                                                </div>
                                                <span class="text-[9px] font-black {{ $persen > 100 ? 'text-red-600' : 'text-gray-600' }}">{{ number_format($persen, 0) }}%</span>
                                            </div>
                                        </td>
                                    </tr>
                                @endforeach
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="p-20 text-center">
                                    <div class="text-gray-300 font-bold text-lg uppercase tracking-widest">Data Tidak Ditemukan</div>
                                    <p class="text-gray-400 text-xs italic">Tidak ada anggaran atau realisasi untuk periode {{ $periodeText }}</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
