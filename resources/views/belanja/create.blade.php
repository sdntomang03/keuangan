<x-app-layout>
    <div class="py-12 bg-gray-50 min-h-screen" x-data="belanjaForm()">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-6 p-5 rounded-2xl shadow-lg text-white flex justify-between items-center {{ $setting->anggaran_aktif == 'BOS' ? 'bg-blue-600' : 'bg-emerald-600' }}">
                <div class="flex items-center gap-4">
                    <div class="p-3 bg-white/20 rounded-xl text-center min-w-[60px]">
                        <p class="text-[10px] font-bold uppercase leading-none">Tahap</p>
                        <p class="text-2xl font-black leading-none mt-1">{{ $setting->triwulan_aktif }}</p>
                    </div>
                    <div>
                        <p class="text-[10px] uppercase font-bold opacity-80 tracking-widest">Input BKU Belanja</p>
                        <h2 class="text-2xl font-black uppercase tracking-tight">
                            {{ $setting->anggaran_aktif }} — TAHUN {{ $setting->tahun_aktif }}
                        </h2>
                    </div>
                </div>
                <div class="text-right border-l border-white/20 pl-6 hidden md:block">
                    <p class="text-sm font-bold">{{ $setting->nama_sekolah }}</p>
                    <p class="text-xs opacity-75 italic">Bendahara: {{ $setting->nama_bendahara }}</p>
                </div>
            </div>
            ...
            <form action="{{ route('belanja.store') }}" method="POST" class="space-y-6">
                ...
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="space-y-4">
                        ...
                        <input type="date" name="tanggal" class="w-full rounded-xl border-gray-200 focus:ring-blue-500" required>
                        <input type="text" name="no_bukti" placeholder="Nomor Bukti" class="w-full rounded-xl border-gray-200" required>
                    </div>
                    <div class="space-y-4 border-l border-gray-100 md:pl-4">
                        ...
                        <select name="rekanan_id" class="w-full rounded-xl border-gray-200" required>
                            <option value="">-- Pilih Rekanan --</option>
                            @foreach($rekanans as $r)
                    ...
                    <div class="space-y-4 border-l border-gray-100 md:pl-4">
                        ...
                        <textarea name="uraian" rows="3" class="w-full rounded-xl border-gray-200 text-sm focus:ring-blue-500" required></textarea>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                    <div class="p-6 bg-gray-50/80 border-b border-gray-200 flex flex-col md:flex-row gap-6">
                        <div class="flex-1">
                            <label class="text-[11px] font-bold text-gray-500 uppercase block mb-2 tracking-widest">1. Pilih Kegiatan</label>
                            <select x-model="selectedIdbl" @change="fetchRekening()" class="w-full rounded-xl border-gray-200">
                        ...
                        <div class="flex-1">
                            <label class="text-[11px] font-bold text-gray-500 uppercase block mb-2 tracking-widest">2. Pilih Kode Akun</label>
                            <select x-model="selectedRekening" @change="fetchKomponen()" :disabled="!rekenings.length" class="w-full rounded-xl border-gray-200">
                                <option value="">-- Pilih Rekening --</option>
                                <template x-for="rek in rekenings" :key="rek.koderekening">
                                    <option :value="rek.koderekening" x-text="rek.koderekening + ' - ' + rek.namarekening"></option>
                                </template>
                            </select>
                        </div>
                    </div>
            ...
            <div class="flex items-center justify-end gap-2">
                ...
                <button type="button" 
                        @click="roundAllPricesDown()" 
                        x-show="items.length > 0"
                        title="Bulatkan semua ke ribuan bawah"
                        class="bg-red-500 hover:bg-red-400 text-white p-1 rounded transition-colors shadow-sm">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" viewBox="0 0 20 20" fill="currentColor">
                        <path fill-rule="evenodd" d="M15.707 15.707a1 1 0 01-1.414 0l-5-5a1 1 0 011.414-1.414L13 11.586V3a1 1 0 112 0v8.586l2.293-2.293a1 1 0 011.414 1.414l-3 3zM3 18a1 1 0 011-1h12a1 1 0 110 2H4a1 1 0 01-1-1z" clip-rule="evenodd" />
                    </svg>
                </button>
            </div>
        </th>
    ...
    <div class="relative flex items-center">
        ...
        <button type="button" 
                ...
                x-show="item.harga_satuan >= 1000"
                title="Bulatkan ke ribuan bawah"
                class="absolute right-1 p-1 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded transition-colors">
    ...
    <div class="mt-1 px-1 flex justify-between items-center">
        <span class="text-[9px] text-gray-400 uppercase font-bold">Pagu ARKAS:</span>
        <span class="text-[10px] text-gray-500 italic font-medium" 
              x-text="formatRupiah(item.harga_asli)">
        </span>
    </div>
</td>
                                        <td class="px-6 py-4 text-right font-bold text-gray-900" x-text="formatRupiah(item.volume * item.harga_satuan)"></td>
                                        <td class="px-6 py-4 text-center">
                                            <button type="button" @click="removeItem(index)" class="text-red-300 hover:text-red-600">&times;</button>
                                        </td>
                                    </tr>
                                </template>
                            </tbody>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 self-start">
                        <div class="flex justify-between items-center mb-6">
                            ...
                            <button type="button" @click="addPajak()" class="text-[10px] bg-orange-100 text-orange-600 px-4 py-1.5 rounded-full font-bold hover:bg-orange-200">+ TAMBAH PAJAK</button>
                        </div>
    ...
    <div class="flex gap-3 mb-4 items-center bg-gray-50 p-3 rounded-xl border border-gray-100">
        <input type="hidden" :name="'pajaks['+pIndex+'][id_master]'" :value="pajak.id_master">
        <input type="hidden" :name="'pajaks['+pIndex+'][nominal]'" :value="pajak.nominal">

        <select x-model="pajak.id_master" 
                @change="calculateTotal()"
                class="flex-1 rounded-xl border-gray-200 text-sm">
            <option value="">-- Pilih Jenis Pajak --</option>
            <template x-for="master in masterPajaks" :key="master.id">
                <option :value="master.id" x-text="master.nama_pajak + ' (' + master.persen + '%)'"></option>
            </template>
        </select>
        
        <div class="w-32 text-sm font-bold text-orange-700 text-right" x-text="formatRupiah(pajak.nominal)"></div>
        
        <button type="button" @click="removePajak(pIndex)" class="text-red-400 font-bold px-2">&times;</button>
    </div>
</template>
                        <p class="text-[10px] text-gray-400 italic mt-4">* Pajak berjenis 'Penambah' (seperti PPN) akan menaikkan nilai Bruto BKU.</p>
                    </div>
                    ...
                    <div class="bg-gray-900 p-8 rounded-3xl shadow-2xl text-white">
                        <div class="space-y-4">
                            ...
                            <div class="flex justify-between items-center bg-blue-600/20 p-4 rounded-2xl border border-blue-500/50 shadow-inner my-4">
                                <div>
                                    <span class="text-[10px] uppercase font-black tracking-[0.2em] block text-blue-300">Total</span>
                                    <span class="text-[9px] text-blue-200 opacity-70">Nilai realisasi</span>
                                </div>
                                <span class="text-3xl font-black text-blue-400" x-text="formatRupiah(totalBruto)"></span>
                            </div>

                        <div class="flex justify-between items-center text-orange-400 border-b border-gray-800 pb-4">
    ...
    <span class="text-xl font-bold" x-text="'- ' + formatRupiah(totalPajak)"></span>
                            ...
                            <div class="pt-4 flex justify-between items-end">
                                <div>
                                    <p class="text-[10px] text-emerald-400 uppercase font-bold tracking-[0.2em] mb-1">Diterima Rekanan (Netto Transfer)</p>
                                    <h3 class="text-4xl font-black text-emerald-400" x-text="formatRupiah(totalTransfer)"></h3>
                                </div>
                                <button type="submit" class="bg-blue-600 hover:bg-blue-500 text-white px-8 py-4 rounded-2xl font-black transition-all transform hover:scale-105 shadow-xl uppercase tracking-tighter">
                                    POSTING BKU
                                </button>
                            </div>
                        </div>
    ...
    <script>
        function belanjaForm() {
            return {
                ...
                addPajak() {
                    this.pajaks.push({ id_master: '', nominal: 0 });
                },
