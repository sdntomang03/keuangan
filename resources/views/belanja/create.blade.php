<x-app-layout>
    ...
    {{ $anggaran->singkatan == 'BOS' ? 'bg-gradient-to-r from-indigo-600 to-blue-500' : 'bg-gradient-to-r from-emerald-600 to-teal-500' }}">
            ...
            initialName: '{{ $rekanans->where('id', old('rekanan_id'))->first()->nama_rekanan ?? '' }}'
                        ...
                        <div @click="open = !open"
                            class="cursor-pointer border border-gray-200 rounded-xl p-2 mt-4 bg-white text-sm flex justify-between items-center">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            <div class="p-6 bg-gray-50/80 border-b border-gray-200 grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <select x-model="selectedRekening" @change="fetchKeterangan()" :disabled="!rekenings.length"
                        ...
                        <template x-for="rek in rekenings" :key="rek.koderekening">
                            <option :value="rek.koderekening" x-text="rek.koderekening + ' - ' + rek.namarekening">
                            </option>
                        </template>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select x-model="selectedKeterangan" @change="fetchKomponen()" :disabled="!keterangans.length"
                        class="w-full rounded-xl border-gray-200 text-sm">
                        ...
                        <tr>
                            ...
                            <th class="px-6 py-4 text-right w-44">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <button type="button" @click="roundAllPricesDown()" x-show="items.length > 0"
                                        title="Bulatkan semua harga"
                                ...
                                <td class="px-5 py-4">
                                    <div class="relative flex items-center">
                                        ...
                                        <div class="relative">
                                            ...
                                            <input type="number" step="any" :name="'items[' + index + '][harga_satuan]'"
                    ...
                    <div class="flex gap-3 mb-4 items-center bg-gray-50 p-3 rounded-xl border border-gray-100">
                        ...
                        <select x-model="pajak.id_master" @change="calculateTotal()"
                            ...
                            <template x-for="master in masterPajaks" :key="master.id">
                                <option :value="master.id" x-text="master.nama_pajak + ' (' + master.persen + '%)'">
                                </option>
                            </template>
                        </select>
    ...
    <script>
        ...
        // Daftarkan komponen rekananDropdown
        // config adalah parameter yang dikirim dari HTML (berisi initialId dan initialName)
        Alpine.data('rekananDropdown', (config = {}) => ({
            ...
            // Dijalankan otomatis saat komponen dimuat
            init() {
                this.fetchData();
            },
            ...
            async fetchData() {
                this.isLoading = true;
                try {
                    const response = await fetch(`/api/rekanan?q=${this.search}`);
            ...
            // Fungsi saat rekanan dipilih dari daftar
            selectRekanan(rekanan) {
                this.selectedId = rekanan.id;
                this.selectedName = rekanan.nama_rekanan;
                this.open = false;
                this.search = ''; // Kosongkan pencarian setelah memilih
                this.fetchData(); // Reset daftar kembali ke awal
            }
        }));
