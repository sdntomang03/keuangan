...
<head>
    ...
    <title>Surat Permintaan Harga</title>
    <style>
...
<body>
    ...
    <div style="padding-left: 95px; margin-top: 20px;">
        <p style="text-align: justify; margin: 0 0 15px 0; text-indent: 48px; line-height: 1.6;">
            Berdasarkan kebutuhan sekolah yang tertuang pada Anggaran Biaya Operasional Pendidikan ({{
            $surat->sumber_dana ?? 'BOP' }})
            {{ $surat->periode ?? 'Triwulan I' }} Tahun Anggaran {{ $surat->tahun_anggaran }} dengan Kode Rekening {{
            $surat->kode_rekening }}
            {{ $surat->nama_rekening }} pada kegiatan {{ $surat->nama_kegiatan }} di {{ $sekolah->nama_sekolah }},
            serta Surat Penawaran Kerja Sama dari {{ $rekanan->nama_perusahaan }}.
            Maka dengan ini kami mohon untuk saudara mengirimkan penawaran harga sesuai dengan Komponen Barang / Jasa
            yang kami perlukan sebagai berikut :
        </p>
