    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                {{ __('Matriks Anggaran Kas (AKB)') }} -
                <span class="text-indigo-600 dark:text-indigo-400">
                    {{ strtoupper($anggaran->singkatan ?? $anggaranAktif->singkatan ?? '') }}
                    {{ $anggaran->tahun ?? $anggaranAktif->tahun ?? '' }}
                </span>
            </h2>

            <div
                class="text-xs font-medium px-2 py-1 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 rounded border border-gray-200 dark:border-gray-600">
                Status: {{ $anggaran->nama_anggaran ?? 'Aktif' }}
            </div>
        </div>
    </x-slot>
    ...
    <div class="bg-white p-4 mb-6 rounded-lg shadow-sm border border-gray-200">
        <form action="{{ route('akb.rincian') }}" method="GET" class="flex flex-wrap items-end gap-4">
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('coba.rkas') }}"
                    class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-4 py-2 rounded-md text-sm">Reset</a>
            </div>
            <a href="{{ route('akb.export_excel', request()->all()) }}"
                class="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded-md text-sm font-medium transition flex items-center">
                ...
                <table class="w-full border-collapse border border-gray-300 text-[11px]">
                    <thead class="bg-gray-100 font-bold">
                        <tr>
                            ...
                            <th colspan="4" class="border border-gray-300 px-2 py-1 text-center bg-blue-50">Rincian Per
                                Triwulan</th>
                            @else
                            <th colspan="12" class="border border-gray-300 px-2 py-1 text-center bg-blue-50">Bulan
                                (Rincian AKB)</th>
                            @endif
                        </tr>
                        <tr>
                            ...
                            @endforeach
                            @else
                            @for ($i = 1; $i <= 12; $i++) <th class="border border-gray-300 px-1 py-1 text-center w-20">
                                {{ $i }}</th>
                                @endfor
                                @endif
                        </tr>
                    ...
                    <tbody>
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="border border-gray-300 px-2 py-1">
                                ...
                                <div class="text-[8px] text-neutral-300 bg-slate-600 uppercase">{{ $rkas->korek->singkat
                                    ?? '-' }}</div>
                            </td>
                            ...
                            @endforeach
                            @else
                            @for ($m = 1; $m <= 12; $m++) <td class="border border-gray-300 px-1 py-1 text-right">
                    ...
                    <tfoot class="bg-gray-100 font-bold border-t-2 border-gray-400">
                        <tr>
                            <td colspan="2" class="border border-gray-300 px-2 py-2 text-center uppercase">Total
                                Keseluruhan</td>
                            ...
                            <td class="border border-gray-300 px-2 py-2 text-right text-indigo-700">
                                {{ number_format($dataRkas->flatMap->akbrincis->whereIn('bulan',
                                $months)->sum('nominal'), 0, ',', '.') }}
                            </td>
                            @endforeach
                            @else
                            @for ($m = 1; $m <= 12; $m++) <td
                                class="border border-gray-300 px-1 py-2 text-right text-indigo-700">
